<?php
/**
 * Class Magazil_Related_Product_Output
 *
...
if ( ! function_exists( 'magazil_call_related_product_class' ) ) {
	/**
	 *
	 ...
	 *
	 */
	function magazil_call_related_product_class() {
		// instantiate the class & load everything else
		Magazil_Related_Product::get_instance();
	}
	add_action( 'wp_loaded', 'magazil_call_related_product_class' );
}
if ( ! class_exists( 'Magazil_Related_Product' ) ) {
	/**
	 * Class Magazil_Related_Product
	 */
	class Magazil_Related_Product {
		/**
		...
		protected function __construct() {
			add_action( 'magazil_single_after_product', array( $this, 'output_related_posts' ), 2 );
		}
		/**
		...
		function output_related_posts() {
			...
			while ( $related_posts->have_posts() ) {
				...
				$category = get_the_terms( $related_posts->post->ID, 'product_category' );
				...
				echo '<div class="posts-i-info">';
				if ( $category && !is_wp_error( $category ) ) :
  					echo '<a href="'.get_category_link($category[0]->term_id).'" class="posts-i-ctg">' . $category[0]->name . '</a>';
                endif;
                	echo '
